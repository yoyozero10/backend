...
export class CreateOrderDto {
    ...
    @ApiPropertyOptional({ example: 'COD', enum: ['COD', 'MOCK'], description: 'Phương thức thanh toán' })
    @IsOptional()
    @IsIn(['COD', 'MOCK'], { message: 'Phương thức thanh toán phải là COD hoặc MOCK' })
    paymentMethod?: string;

    @ApiPropertyOptional({ example: 'Giao hàng trong giờ hành chính', description: 'Ghi chú đơn hàng' })
    @IsOptional()
    @IsString()
    note?: string;
}
